...
export default function FrenPetPage() {
  ...
  useEffect(() => {
    ...
    if (latestEvent) {
      ...
      } else if (latestEvent.eventName === 'BattleResult') {
          ...
          {events.slice(-5).reverse().map((event, idx) => (
            <div key={idx} className="text-sm">
              {event.eventName === 'PetCreated' && (
                <span>🥚 New pet created: {event.args.name}</span>
              )}
              {event.eventName === 'BattleResult' && (
                <span>⚔️ Battle won by {event.args.winner.slice(0, 6)}...</span>
              )}
              {event.eventName === 'PetLevelUp' && (
                <span>⬆️ Pet leveled up to {event.args.newLevel}</span>
              )}
            </div>
          ))}
        </div>
