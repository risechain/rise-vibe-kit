  ...
  useEffect(() => {
    ...
    if (latestEvent) {
      if (latestEvent.eventName === 'TokenLaunched') {
        ...
        toast.info(`Someone ${action} ${formatEther(latestEvent.args.tokenAmount)} tokens`);
        loadActiveTokens();
      }
    }
  }, [events]);
  ...
  const loadActiveTokens = async () => {
    ...
    const tokenDetails = await Promise.all(
      tokens.map(async (addr) => {
        ...
        return { ...info, currentPrice: price };
      })
    );
          ...
          {events.slice(-10).reverse().map((event, idx) => (
            <div key={idx} className="text-sm">
              {event.eventName === 'TokenLaunched' && (
                <span>🚀 <b>{event.args.name}</b> launched!</span>
              )}
              {event.eventName === 'TokenTraded' && (
                <span>
                  💰 {event.args.isBuy ? 'Buy' : 'Sell'}: {formatEther(event.args.tokenAmount)} tokens
                </span>
              )}
            </div>
          ))}
        ...
        {activeTokens.map((token) => (
          <Card key={token.tokenAddress} className="p-6 hover:shadow-lg transition-shadow">
            <div className="flex items-start justify-between mb-4">
              ...
              {token.imageUrl && (
                <img src={token.imageUrl} alt={token.name} className="w-12 h-12 rounded" />
              )}
            </div>
